...
export async function POST(request: Request) {
  try {
    ...
    if (typeof resume !== 'object' || resume === null) {
      return NextResponse.json({ error: 'Resume must be an object' }, { status: 400 });
    }
    ...
    if (focusAreas && !focusAreas.every((area: unknown) => typeof area === 'string')) {
      return NextResponse.json(
        { error: 'focusAreas must be an array of strings' },
        { status: 400 },
      );
    }
